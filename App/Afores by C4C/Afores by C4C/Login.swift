...
class Login: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()/*
        let imagData = NSData(contentsOfURL: NSBundle.mainBundle().URLForResource("abr-24-2016 10-53-51", withExtension: "gif")!)
        let imageGif = UIImage.gifWithData(imagData!)
        let imageView = UIImageView(image: imageGif)
        imageView.frame = CGRect(x: 0.0, y:0.0, width:320.0, height: 568.0)
